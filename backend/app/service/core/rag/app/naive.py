...
# app\service\core\rag\app\naive.py

current_dir = os.path.dirname(current_file)          # .../service/core/rag/app
rag_dir = os.path.dirname(current_dir)               # .../service/core/rag/
core_dir = os.path.dirname(rag_dir)                  # .../service/core/
service_dir = os.path.dirname(core_dir)           # .../service/
...
# from utils.load_nltk   import load_nltk_path
# load_nltk_path()# 导入nltk路径
...
def chunk(filename, binary=None, from_page=0, to_page=100000,
    ...
    st = timer()
    chunks = naive_merge(              # # 这才是真正的“切片”动作
        sections, int(parser_config.get(
            "chunk_token_num", 128)), parser_config.get(
            "delimiter", "\n!?。；！？"))
    if kwargs.get("section_only", False):
...
#     chunk(r"D:\studySpace\projectAI\swxy\backend\test_pdf.pdf", from_page=0, to_page=10, callback=dummy)
...
if __name__ == "__main__":
    ...
    # from app.service.core.rag.nlp import naive_merge
    sections = [
    ("这是第⼀段，内容较短。", "(p1)"),
    ("这是第⼆段，它稍微⻓⼀些，需要特别关注。", "(p1)"),
    ("接下来是第三段...", "(p1)")
    ]
