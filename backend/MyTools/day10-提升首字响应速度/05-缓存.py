...
# ================= 完美启动方案 =================
print("正在清理历史环境残留...")
default_server.cleanup()  # <--- 关键新增：每次启动前先清空旧垃圾，防止卡死
...
# stream_chat第一版
async def stream_chat1(messages):
    """
    使用百炼文本模型流式生成。
...
async def stream_chat(messages):
    ...
    async for chunk in response:
        if chunk.choices and chunk.choices[0].delta.content:
            content = chunk.choices[0].delta.content
            # 💡 核心就在这一句：必须是 yield，不能是 print！
            yield content


# 第一版
async def rag_once1(question, k=3):
...
async def rag_once(question, k=3):
    ...
    print("\n[AI] ", end="", flush=True)
    # 因为 stream_chat 变成了水龙头，这里也得用 async for 接水
    async for tok in stream_chat(prompt):
...
def _hash(text: str) -> str:
    """计算文本的哈希值，用于生成独一无二的 Redis Key"""
    return hashlib.sha1(" ".join(text.split()).lower().encode()).hexdigest()

def _get(key: str):
    """从 Redis 读取数据并反序列化"""
    if val := redis_cli.get(key):
        return json.loads(val)
    return None

def _set(key: str, val, ttl: timedelta):
    """序列化数据并存入 Redis，设置过期时间"""
    redis_cli.set(key, json.dumps(val), ex=int(ttl.total_seconds()))
...
async def run_cache_performance_test():
    # 找一个稍微长一点、且你之前绝对没有问过的新问题
    test_question = "请详细解释一下什么是量子纠缠，以及它在未来通信领域有哪些潜在的应用场景？"
    
    # 【严谨起见】为了保证第一次绝对是“未命中”，我们主动把这个词的缓存从 Redis 删掉
    ans_key = f"ans:{_hash(test_question)}"
    redis_cli.delete(ans_key)
    ...
    # ---------------- 第一次请求（缓存未命中） ----------------
    print("▶️ [第 1 次请求] 预期：缓存未命中，调用大模型现算...")
    start_time_1 = time.perf_counter() # 开始计时
    
    await answer_cached(test_question)
    
    end_time_1 = time.perf_counter()   # 结束计时
    cost_miss = end_time_1 - start_time_1
    print(f"\n⏱️ 第 1 次耗时 (Miss): {cost_miss:.4f} 秒\n")
    print("-" * 50 + "\n")


    # ---------------- 第二次请求（缓存命中） ----------------
    print("▶️ [第 2 次请求] 预期：命中 Redis 缓存，直接秒回...")
    start_time_2 = time.perf_counter() # 开始计时
    
    await answer_cached(test_question)
    
    end_time_2 = time.perf_counter()   # 结束计时
    cost_hit = end_time_2 - start_time_2
    print(f"\n⏱️ 第 2 次耗时 (Hit):  {cost_hit:.4f} 秒\n")


    # ---------------- 性能总结 ----------------
    print("==================================================")
    print("📊 性能对比总结")
    print(f"   未命中耗时: {cost_miss:.4f} 秒")
    print(f"   命中耗时:   {cost_hit:.4f} 秒")
    if cost_hit > 0:
        print(f"🚀 速度提升了约: {cost_miss / cost_hit:.1f} 倍！")
    print("==================================================")
